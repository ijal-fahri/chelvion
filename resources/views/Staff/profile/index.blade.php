...
<body>
    <div class="relative min-h-screen md:flex">
        ...
        {{-- [INI PERBAIKANNYA] Memanggil sidebar dari folder 'Staff' --}}
        @include('Staff.partials.sidebar')

        <div class="flex-1 md:ml-64 flex flex-col">
            <main class="flex-grow p-4 sm:p-6 lg:p-8 animate-fade-in">
                
                {{-- [INI PERBAIKANNYA] Memanggil header dari folder 'Staff' --}}
                    ...
                    <div class="lg-col-span-1 space-y-8">
                        <div class="bg-white p-6 rounded-2xl shadow-lg">
                            <div class="flex flex-col items-center text-center">
                                <div class="relative">
                                    {{-- [DINAMIS] Avatar berdasarkan nama user --}}
                                ...
                                {{-- [DINAMIS] Nama dan Email dari user yang login --}}
                            ...
                            <div class="space-y-4 text-sm">
                                <div class="flex justify-between">
                                    ...
                                    {{-- [DINAMIS] Menampilkan usertype --}}
                                    <span class="text-gray-800 text-right capitalize">{{ str_replace('_', ' ', Auth::user()->usertype) }}</span>
                                </div>
                                <div class="flex justify-between">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    {{-- [DINAMIS] Menampilkan tanggal bergabung --}}
    ...
    <div id="edit-profile-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 hidden z-50">
        <div class="modal-content bg-white w-full max-w-md p-6 rounded-2xl shadow-xl transform scale-95">
            ...
            {{-- [DINAMIS] Form mengarah ke route profile.update --}}
            <form id="edit-profile-form" action="{{ route('profile.update') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ Auth::user()->name }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-600">Alamat Email</label>
                    <input type="email" id="email" name="email" value="{{ Auth::user()->email }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" readonly>
                </div>
                <div>
                    <label for="telepon" class="block mb-2 text-sm font-medium text-gray-600">Nomor Telepon</label>
                    <input type="tel" id="telepon" value="081234567890" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#edit-profile-form').submit(function(e) {
                // Biarkan form submit secara normal ke route('profile.update')
                // e.preventDefault();
                // hideModal();
                // Swal.fire({
